...
class WOOMULTI_CURRENCY_F_Data {
	...
	public function __construct() {
		...
		if ( ! $wmc_settings ) {
			...
			$wmc_settings['currency_pos'] = get_option( 'woocommerce_currency_pos' );
		}
		...
		$args         = array(
			...
			'currency_pos'               => array($wmc_settings['currency_pos']),
			'auto_detect'                => 0,
			...
			foreach ( $selected_currencies as $k => $currency ) {
				...
				if ( $current_currency == $k ) {
					if ( isset( $_GET['min_price'] ) ) {
						$arg['min_price'] = floatval($_GET['min_price']);
					}
					if ( isset( $_GET['max_price'] ) ) {
						$arg['max_price'] = floatval($_GET['max_price']);
					}
				} else {
					if ( isset( $_GET['min_price'] ) ) {
						$arg['min_price'] = ( floatval($_GET['min_price']) / $selected_currencies[ $current_currency ]['rate'] ) * $currency['rate'];
					}
					if ( isset( $_GET['max_price'] ) ) {
						$arg['max_price'] = ( floatval($_GET['max_price']) / $selected_currencies[ $current_currency ]['rate'] ) * $currency['rate'];
					}
				}
